...
public class HanpayParser implements CardParser {
    ...
    // BALANCE_RECORD_HANPAY = { 0, -78, X, 36, 26 } = 0x00, 0xB2, X, 0x24, 0x1A (SFI 4)
    // TRANS_RECORD_HANPAY = { 0, -78, X, 28, 26 } = 0x00, 0xB2, X, 0x1C, 0x1A (SFI 3)
    ...
    private int readBalance(IsoDep isoDep) {
        try {
            Log.d(TAG, "readBalance: Starting Hanpay balance read...");
        ...
        try {
            ...
            if (response != null && response.length > 2) {
                ...
                if (sw1 == 0x90 && sw2 == 0x00) {
                    ...
                    Log.d(TAG, "readCardNumber: GET RESPONSE: " + bytesToHex(response));
                    if (response != null && response.length > 2) {
                        String cardNum = findCardNumberInData(response, response.length - 2);
                        if (cardNum != null) {
                            Log.i(TAG, "readCardNumber: Found from GET RESPONSE = " + cardNum);
                            return cardNum;
                        }
                    }
                }
