...
public class CashbeeParser implements CardParser {
    ...
    // CARDINFO_CASHBEE = { 0, -78, 1, 20, 51 } = 0x00 0xB2 0x01 0x14 0x33 (READ RECORD SFI 2, Record 1, Le=51)
    private static final byte[] CMD_CARDINFO = {0x00, (byte) 0xB2, 0x01, 0x14, 0x33};

    // BALANCE_RECORD_CASHBEE = { 0, -78, X, 36, 26 } = SFI 4 (P2=0x24), Le=26
    // TRANS_RECORD_CASHBEE = { 0, -78, X, 28, 26 } = SFI 3 (P2=0x1C), Le=26
    ...
    public TransitCardData parse(IsoDep isoDep, byte[] cardId) {
        try {
            ...
            return new TransitCardData(CardType.CASHBEE, cardNumber, balance, transactions);
        } catch (Exception e) {
            Log.e(TAG, "Error parsing Cashbee card", e);
            return new TransitCardData(CardType.CASHBEE, bytesToHex(cardId), 0, new ArrayList<>());
        }
    }
    ...
    private String readCardNumber(IsoDep isoDep) {
        ...
        // CARDINFO_CASHBEE 명령 (SFI 2, Record 1)
        try {
    ...
    private List<Transaction> readTransactionHistory(IsoDep isoDep) {
        ...
        // BALANCE_RECORD (SFI 4) - 위에서 못찾은 경우
