...
public class RailplusParser implements CardParser {
    ...
    // Rail+ is T-money compatible, using T-money style SFI records for transaction history
    // BALANCE_RECORD: SFI 4 (P2=0x24)
    // TRANS_RECORD: SFI 3 (P2=0x1C)
    private static final byte P2_BALANCE_RECORD = 0x24;  // SFI 4
    private static final byte P2_TRANS_RECORD = 0x1C;    // SFI 3
    private static final byte LE_RECORD = 0x2E;          // 46 bytes (T-money style)
    ...
    private int readBalance(IsoDep isoDep) {
        try {
            Log.d(TAG, "readBalance: Starting Railplus balance read...");
    ...
    private List<Transaction> readTransactionHistory(IsoDep isoDep) {
        ...
        // 방법 2: TRANS_RECORD 읽기 (SFI 3) - 거래 내역
        ...
        // 방법 3: BALANCE_RECORD 읽기 (SFI 4) - 잔액 변동 내역
        if (transactions.isEmpty()) {
            for (int record = 1; record <= 10; record++) {
                try {
        ...
        // 방법 4: SFI 기반 READ RECORD (일반적인 SFI: 7, 8, 9, 10, 11, 12)
        if (transactions.isEmpty()) {
            Log.d(TAG, "readTransactionHistory: Trying additional SFI method...");
